...
CONFIG = [
    # metadata strings
    'host', 'grpc-timeout', 'grpc-internal-encoding-request',
    'grpc-internal-stream-encoding-request', 'grpc-payload-bin', ':path',
    'grpc-encoding', 'grpc-accept-encoding', 'user-agent', ':authority',
    'grpc-message', 'grpc-status', 'grpc-server-stats-bin', 'grpc-tags-bin',
    'grpc-trace-bin', 'grpc-previous-rpc-attempts', 'grpc-retry-pushback-ms',
    '1', '2', '3', '4', '',
    # channel arg keys
    'grpc.wait_for_ready', 'grpc.timeout', 'grpc.max_request_message_bytes',
    'grpc.max_response_message_bytes',
    # well known method names
    '/grpc.lb.v1.LoadBalancer/BalanceLoad',
    # compression algorithm names
    'deflate', 'gzip', 'stream/gzip',
    # metadata elements
    ('grpc-status', '0'), ('grpc-status', '1'), ('grpc-status', '2'),
    ('grpc-encoding', 'identity'), ('grpc-encoding', 'gzip'),
    ('grpc-encoding', 'deflate'), ('te', 'trailers'),
    ('content-type', 'application/grpc'), (':method', 'POST', 3),
    (':status', '200', 8), (':status', '404', 13), (':scheme', 'http', 6),
    (':scheme', 'https', 7), (':scheme', 'grpc', 0), (':authority', '', 1),
    (':method', 'GET', 2), (':method', 'PUT'), (':path', '/', 4),
    (':path', '/index.html', 5), (':status', '204', 9), (':status', '206', 10),
    (':status', '304', 11), (':status', '400', 12), (':status', '500', 14),
    ('accept-charset', '', 15), ('accept-encoding', ''),
    ('accept-encoding', 'gzip, deflate', 16), ('accept-language', '', 17),
    ('accept-ranges', '', 18), ('accept', '', 19),
    ('access-control-allow-origin', '', 20), ('age', '', 21), ('allow', '', 22),
    ('authorization', '', 23), ('cache-control', '', 24),
    ('content-disposition', '', 25), ('content-encoding', 'identity'),
    ('content-encoding', 'gzip'), ('content-encoding', '', 26),
    ('content-language', '', 27), ('content-length', '', 28),
    ('content-location', '', 29), ('content-range', '', 30),
    ('content-type', '', 31), ('cookie', '', 32), ('date', '', 33),
    ('etag', '', 34), ('expect', '', 35), ('expires', '', 36), ('from', '', 37),
    ('host', '', 38), ('if-match', '', 39), ('if-modified-since', '', 40),
    ('if-none-match', '', 41), ('if-range', '', 42),
    ('if-unmodified-since', '', 43), ('last-modified', '', 44), ('lb-token',
                                                                 ''),
    ('lb-cost-bin', ''), ('link', '', 45), ('location', '', 46),
    ('max-forwards', '', 47), ('proxy-authenticate', '', 48),
    ('proxy-authorization', '', 49), ('range', '', 50), ('referer', '', 51),
    ('refresh', '', 52), ('retry-after', '', 53), ('server', '', 54),
    ('set-cookie', '', 55), ('strict-transport-security', '', 56),
    ('transfer-encoding', '', 57), ('user-agent', '', 58), ('vary', '', 59),
    ('via', '', 60), ('www-authenticate', '', 61),
]
...
METADATA_BATCH_CALLOUTS = [
    # (name)
    (':path'), (':method'), (':status'), (':authority'), (':scheme'), ('te'),
    ('grpc-message'), ('grpc-status'), ('grpc-payload-bin'), ('grpc-encoding'),
    ('grpc-accept-encoding'), ('grpc-server-stats-bin'), ('grpc-tags-bin'),
    ('grpc-trace-bin'), ('content-type'), ('content-encoding'),
    ('accept-encoding'), ('grpc-internal-encoding-request'),
    ('grpc-internal-stream-encoding-request'), ('user-agent'), ('host'),
    ('lb-token'), ('grpc-previous-rpc-attempts'), ('grpc-retry-pushback-ms'),
]

COMPRESSION_ALGORITHMS = ['identity', 'deflate', 'gzip',]

STREAM_COMPRESSION_ALGORITHMS = ['identity', 'gzip',]


# utility: mangle the name of a config
def mangle(elem, name=None):
    xl = {'-': '_', ':': '', '/': 'slash', '.': 'dot', ',': 'comma', ' ': '_',}
