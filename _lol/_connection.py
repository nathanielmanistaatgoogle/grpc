...
class _Pipe(_common.ChannelRpcHandler, _common.ServerRpcHandler):
    ...
    def initial_metadata(self):
        logging.warning('_Pipe.initial_metadata: Entered!')
            ...
            while True:
                if self._initial_metadata is None:
                    if self._code is None and not self._cancelled:
                        ...
                        logging.warning('_Pipe.initial_metadata: returning phony empty metadata!')
                        return _common.FUSSED_EMPTY_METADATA
                else:
                    logging.warning('_Pipe.initial_metadata: returning real initial metadata!')
                    return self._initial_metadata

    def add_request(self, request):
        logging.warning('_Pipe.add_request: Entered!')
        with self._condition:
            if self._no_more_requests:
                ...
                logging.warning('_Pipe.add_request: Exiting!')
                return True
    ...
    def terminate(self):
        logging.warning('_Pipe.terminate: Entered!')
        with self._condition:
            while True:
                if self._code is None:
                    self._condition.wait()
                else:
                    logging.warning('_Pipe.terminate: Leaving!')
    ...
    def take_request(self):
        logging.warning('_Pipe.take_request: Entered!')
        with self._condition:
            while True:
                if self._cancelled or self._code is not None:
                    logging.warning('_Pipe.take_request: returning TERMINATED.')
                    return _common.TERMINATED
                elif self._requests:
                    logging.warning('_Pipe.take_request: returning a request!')
                    request = self._requests.pop(0)
                    self._condition.notify_all()
                    return _common.ServerRpcRead(request, False, False)
                elif self._no_more_requests:
                    logging.warning('_Pipe.take_request: returning NO_MORE_REQUESTS')
...
class _Connection(grpc_testing.Connection):
    ...
    def __init__(self, servicers_to_descriptions, time):
        ...
        self._channel_fixture = _channel.ChannelFixture(
            tuple(servicers_to_descriptions), _ChannelHandler(server_fixturish),
            time)
