...
def _transform_many(exception_message, transformer, values):
    if transformer is None:
        ...
        return _really_transform(
            exception_message, functools.partial(map, transformer), values)


serialize_request = functools.partial(
    _transform, 'Exception serializing request!')
deserialize_request = functools.partial(
    _transform, 'Exception deserializing request!')
serialize_response = functools.partial(
    _transform, 'Exception serializing response!')
deserialize_response = functools.partial(
    _transform, 'Exception deserializing response!')
serialize_requests = functools.partial(
    _transform_many, 'Exception serializing requests!')
serialize_responses = functools.partial(
    _transform_many, 'Exception serializing responses!')


def method_description(service_name, method_name, descriptions):
...
def fuss_with_metadata(application_metadata):
  return tuple(
      ([] if application_metadata is None else list(application_metadata)) +
      [
...
class ServerRpcRead(
    collections.namedtuple(
        'ServerRpcRead', ('request', 'no_more_requests', 'terminated',))):
    pass
...
class ServerFixtureish(six.with_metaclass(abc.ABCMeta)):
    ...
    def invoke_rpc_by_service_and_method_names(
        self, service_name, method_name, handler, invocation_metadata, timeout):
        """"""
        raise NotImplementedError()
