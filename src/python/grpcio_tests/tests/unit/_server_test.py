...
class ServerTest(unittest.TestCase):
    ...
    def test_not_a_generic_rpc_handler_at_construction(self):
        with self.assertRaises(AttributeError) as exception_context:
            grpc.server(futures.ThreadPoolExecutor(max_workers=5), handlers=[_ActualGenericRpcHandler(), object(),])
    ...
    def test_not_a_generic_rpc_handler_after_construction(self):
        ...
        with self.assertRaises(AttributeError) as exception_context:
            server.add_generic_rpc_handlers([_ActualGenericRpcHandler(), object(),])
        self.assertIn('grpc.GenericRpcHandler', str(exception_context.exception))
