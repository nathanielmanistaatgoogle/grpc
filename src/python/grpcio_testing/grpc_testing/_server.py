...
class _RequestIterator(object):
    ...
    def _next(self):
        ...
        if read.no_more_requests:
            ...
            return _common.deserialize_request(self._request_deserializer,
                                               read.request)[0]
...
def _expire(state, handler):
    with state.condition:
        _abort(state, handler, grpc.StatusCode.DEADLINE_EXCEEDED,
               'Took too much time!')
...
class _ServicerContext(grpc.ServicerContext):
        ...
        with self._state.condition:
            if self._state.callbacks is None:
                ...
                if client_active:
                    ...
                    _abort(self._state, self._handler, grpc.StatusCode.UNKNOWN,
    ...
    def cancel(self):
        with self._state.condition:
            _abort(self._state, self._handler, grpc.StatusCode.CANCELLED,
...
def _unary_request(state, handler, request_deserializer):
    ...
    if read.no_more_requests:
        with state.condition:
            _abort(state, handler, grpc.StatusCode.UNIMPLEMENTED,
                   'Cardinality violation: needed one request; got zero!')
        return None, False
    elif read.terminated:
        with state.condition:
            _abort(state, handler, grpc.StatusCode.UNKNOWN,
                   'Client no longer active!')
        return None, False
    else:
        return _common.deserialize_request(request_deserializer,
                                           read.request)[0], True


def _unary_response(argument, implementation, state, handler, servicer_context,
                    response_serializer):
    try:
...
def _stream_response(argument, implementation, state, handler, servicer_context,
                     response_serializer):
    try:
        ...
        while True:
            try:
                ...
                with state.condition:
                    _ensure_initial_metadata_sent(state, handler)
                    handler.add_response(
                        _common.serialize_response(response_serializer,
                                                   response)[0])


def _service_unary_unary(implementation, state, handler, servicer_context,
                         request_deserializer, response_serializer):
    request, proceed = _unary_request(state, handler, request_deserializer)
    if proceed:
        _unary_response(request, implementation, state, handler,
                        servicer_context, response_serializer)


def _service_unary_stream(implementation, state, handler, servicer_context,
                          request_deserializer, response_serializer):
    request, proceed = _unary_request(state, handler, request_deserializer)
    if proceed:
        _stream_response(request, implementation, state, handler,
                         servicer_context, response_serializer)


def _service_stream_unary(implementation, state, handler, servicer_context,
                          request_deserializer, response_serializer):
    _unary_response(
        _RequestIterator(state, handler, request_deserializer), implementation,
        state, handler, servicer_context, response_serializer)


def _service_stream_stream(implementation, state, handler, servicer_context,
                           request_deserializer, response_serializer):
    _stream_response(
        _RequestIterator(state, handler, request_deserializer), implementation,
        state, handler, servicer_context, response_serializer)
...
def _service(description, implementation, state, handler, time):
    ...
    if description.request_unary:
        if description.response_unary:
            _service_unary_unary(implementation, state, handler,
                                 servicer_context, request_deserializer,
                                 response_serializer)
        else:
            _service_unary_stream(implementation, state, handler,
                                  servicer_context, request_deserializer,
                                  response_serializer)
    else:
        if description.response_unary:
            _service_stream_unary(implementation, state, handler,
                                  servicer_context, request_deserializer,
                                  response_serializer)
        else:
            _service_stream_stream(implementation, state, handler,
                                   servicer_context, request_deserializer,
                                   response_serializer)


def _description_and_implementation(servicers_to_descriptions, service_name,
                                    method_name):
...
class _RpcRpcHandler(_common.ServerRpcHandler, grpc_testing.ServerRpc):
    ...
    def __init__(self, requests, no_more_requests, request_serializer,
    ...
    def add_requests_as_messages(self, requests):
        with self._condition:
            self._requests.extend(
                _common.serialize_requests(self._request_serializer, requests)[
                    0])
            self._condition.notify_all()
...
class _Fixtureish(_common.ServerFixtureish):
    ...
    def invoke_rpc_by_service_and_method_names(self, service_name, method_name,
        ...
        service_thread = threading.Thread(
            target=_service,
            args=(method_description, implementation, state, handler,
                  self._time,))
