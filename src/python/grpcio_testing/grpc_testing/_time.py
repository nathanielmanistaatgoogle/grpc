...
def _call(behaviors):
    for behavior in behaviors:
        try:
            ...
            logging.exception('Exception calling behavior?')
    ...
    while state.times_to_behaviors:
        ...
        if earliest_time <= now:
            ...
            earliest_mature_behaviors = state.times_to_behaviors.pop(
                earliest_time)
            for mature_behavior in earliest_mature_behaviors:
                state.behaviors_to_times.pop(mature_behavior)
...
class _Future(grpc.Future):
    ...
    def cancel(self):
        with self._state.condition:
            time = self._state.behaviors_to_times.pop(self._behavior, None)
            if time is None:
                return False
            else:
                self._state.times_to_behaviors[time].remove(self._behavior)
                if not self._state.times_to_behaviors[time]:
                    self._state.times_to_behaviors.pop(time)
...
class RealTime(grpc_testing.Time):
    ...
    def _ensure_called_through(self, time):
        with self._state.condition:
            while (
                (self._state.times_to_behaviors and
                 min(self._state.times_to_behaviors) < time)
                or
                (self._calling is not None and
                 self._calling < time)):
                self._state.condition.wait()

    def _call_at(self, behavior, time):
        with self._state.condition:
            self._state.behaviors_to_times[behavior] = time
            ...
            if self._active:
                ...
                activity.start()
                self._active = True
            return _Future(self._state, behavior)
...
class FakeTime(grpc_testing.Time):
